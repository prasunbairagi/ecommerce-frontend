...
export default function AddProduct() {
	...
	return (
		<>
		{error && <ErrorMsg message={error} onConfirm={() => {dispatch(resetErrAction())}}/>}
		{fileErrs?.length > 0 && <ErrorMsg message='file too large or upload an image' />}

		{isAdded && <SuccessMsg message="Product Added Successfully" onConfirm={() => {dispatch(resetSuccessAction())}} />}
		<div className="flex min-h-full flex-col justify-center py-12 sm:px-6 lg:px-8">
			...
			<div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
				...
				<div>
					...
					<button disabled={fileErrs?.length > 0}
						type="submit"
